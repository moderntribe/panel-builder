			...
			<select class="select-new-filter">
				...
				<optgroup label="<?php esc_attr_e('Taxonomy', 'modular-content'); ?>">
					<?php foreach ( $taxonomies as $tax_name ): ?>
						...
						<?php if ( !$tax ) { continue; }
						$connected_post_types = json_encode( \ModularContent\Util::get_post_types_for_taxonomy( $tax_name ) );
						?>
				...
				<?php if ( $p2p ){
					...
					foreach ( $p2p as $relationship_id => $relationship ) {
						$connected_post_types = json_encode( \ModularContent\Util::get_post_types_for_p2p_relationship( $relationship ) );
